...
const App = () => {
  ...
  const handleMessage = async (event: {nativeEvent: {data: string}}) => {
    ...
    if (message.type === 'INIT_LOAD_GEOJSON') {
      const data = await readGeoJSONFile(
        `${RNFS.DownloadDirectoryPath}/wg-survey/geojson/survey.geojson`,
      ); // 앱의 외부 저장소 경로를 사용
      try {
        console.log('INIT_LOAD_GEOJSON');
  ...
  const displayGeoJSONOnMap = (
    geojsonData: Geometry[] | null,
    shpFileName: string,
    folderName: string,
  ) => {
    if (geojsonData) {
  ...
  return (
    <View style={{flex: 1}}>
      <WebView
        ...
        originWhitelist={['*']}
        source={{uri: 'file:///android_asset/login.html'}} // login.html을 초기 페이지로 설정
        onMessage={handleMessage}
        onLoad={checkFirstLaunch} // 로그인 페이지가 로드될 때 권한을 체크하도록 설정
