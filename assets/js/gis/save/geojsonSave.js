export const saveFeature = () => {
    ...
    for(let i=0; i<document.getElementsByClassName("surveyKey").length; i++){
          ...
          console.log(document.getElementsByClassName("surveyKey")[i].innerText);
          console.log(document.getElementsByClassName("surveyValue")[i].value);
  
          modifyInteraction.source_.getFeatures()[0].values_[document.getElementsByClassName("surveyKey")[i].innerText] = document.getElementsByClassName("surveyValue")[i].value;
    }
          // GeoJSON 문자열로 변환
